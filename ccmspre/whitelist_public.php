...
define('EMAIL', '/^[a-z0-9%+-._]+@[a-z0-9-.]+\.[a-z0-9-]{2,20}\z/i');
//define('EMAIL', '/^[\pL\pM*+\pN._%+-]+@[\pL\pM*+\pN.-]+\.[\pL\pM*+-]{2,20}\z/u');
...
function CCMS_Public_Filter($input, $whitelist) {
	...
	foreach($input as $key => $value) {
		if(array_key_exists($key, $whitelist)) {
			...
			if($value != "MINLEN" && $value != "MAXLEN" && $value != "") {
				switch($whitelist[$key]['type']) {


					case "EXAMPLE_EXPRESSION_1":
						$value = (preg_match(EXAMPLE_EXPRESSION_1, $value)) ? $value : "INVAL";
						break;
					case "EXAMPLE_EXPRESSION_2":
						$value = (preg_match(EXAMPLE_EXPRESSION_2, $value)) ? $value : "INVAL";
						break;
					case "NO_BADCHARS":
						$value = (preg_match(NO_BADCHARS, $value)) ? $value : "INVAL";
						break;
					case "EMAIL":
						$value = (preg_match(EMAIL, $value)) ? $value : "INVAL";
						break;
